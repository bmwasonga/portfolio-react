export const projList = [
  {
    id: 1,
    url: 'https://socks-globe.herokuapp.com/',
    text: 'Socks-World',
    image: '/images/projects/socks.png',
    desc: 'An e-comerce website made with React, MongoDb and Redux. Deployed on Heroku.',
  },
  {
    id: 2,
    url: 'https://mystifying-movieapp-20.netlify.app/',
    text: 'Movie-app',
    image: '/images/projects/movie-app.png',
    desc: 'This is a movie app that sources data from The movieDb website.',
  },
  {
    id: 3,
    url: 'https://github.com/bmwasonga/disney-plus/',
    text: 'Disney + clone',
    image: '/images/projects/disney.png',
    desc: 'A working clone of disney+ movie website made with Reactjs and styled components',
  },
  {
    id: 4,
    url: 'https://github.com/bmwasonga/netflix-clone/',
    text: 'Netflix Clone',
    image: '/images/projects/netflix.png',
    desc: 'A clone of the netflix application made with React and firebase',
  },
  {
    id: 5,
    url: 'https://wasongatodo.netlify.app/',
    text: 'To-Do-List',
    image: '/images/projects/to-do.png',
    desc: "A webapp that tracks your daily to-do's",
  },
  {
    id: 6,
    url: 'https://about-ben-accordion.netlify.app/',
    text: 'Accordion',
    image: '/images/projects/accordion.png',
    desc: 'A simple accordion that takes static data to give information about me',
  },
];
/* the Text will contain the name of the project will serve to
 provide basic information on the project before the user choses 
 to click on it*/
